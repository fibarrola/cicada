...
img = image_loader('data/test_hat_11.png').squeeze(0)
...
for i in range(3):
    for j in range(3):
        ...
        losses = [
            '\\' + 'textcolor{ }{' + '{loss:1.3f}'.format(loss=loss[l].item()) + '}, '
            for l in loss
        ]
        string = ''
        for l in losses:
...
#     if t % 50 == 0:
#         cl = clipConvLoss(img, img0.permute(2,0,1).unsqueeze(0))
#         print([cl[x].item() for x in cl])

...
for l in range(len(style_features[0][0])):
    ...
    (i2, j2) = (0, 1)
    shape_change = round(
        torch.norm(style_features[i1][j1][l] - style_features[i2][j2][l]).item()
    )
    (i1, j1) = (0, 0)
    (i2, j2) = (1, 0)
    color_change = round(
    ...
    print(
        ...
        color_change,
        multi_change,
        multi_change > color_change and multi_change > shape_change,
    )
...
for l in range(len(style_features[0])):
    ...
    for j in range(1, 4):
        vals.append(
            round(torch.norm(style_features[0][l] - style_features[j][l]).item())
        )
    print(vals)
...
for img_name in [
    'chair01.jpeg',
    'chair_clip_and_me_64_bf.jpeg',
    'chair_clip_and_me_128_bh.jpeg',
    'chair_clip_and_me_128_bl.jpeg',
]:
